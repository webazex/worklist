...
//    ====search-start
    ...
    static function db_search_numbers($field, $value){
        $status = self::db_status();
        if($status == false):
            throw new \Exception('Немає з’єднання з базою данних!');
        else:
            $arRezult = array();
            $bd = self::db_connect();
            $bd->real_query("SELECT * FROM `tasks` WHERE `".$field."` LIKE '%".$value."%'");
            $ret = $bd->use_result();
            while ($row = $ret->fetch_row()):
                array_push($arRezult, $row);
            endwhile;
    ...
    static function db_search_department_performers($value){
        $status = self::db_status();
        if($status == false):
            throw new \Exception('Немає з’єднання з базою данних!');
        else:
            $arRezult = array();
            $bd = self::db_connect();
            $bd->real_query("SELECT * FROM `tasks` WHERE `performers_departament` LIKE '%".$value."%'");
            $ret = $bd->use_result();
            while ($row = $ret->fetch_row()):
                array_push($arRezult, $row);
            endwhile;
